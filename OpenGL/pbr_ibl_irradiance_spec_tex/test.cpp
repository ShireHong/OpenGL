...
int main()
{
    ...
    // 为预过滤映射中较低的mip级别启用无缝立方映射采样。
    glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS);

	Shader pbrshader("pbr.vs","pbr.fs");
    Shader ctangulartoCubemap("cubemap.vs","ctangulartoCubemap.fs");
    Shader irradiance("cubemap.vs","irradiance_jj.fs");
    Shader prefilter("cubemap.vs", "prefilter.fs");
    Shader brdf("brdf.vs", "brdf.fs");
    Shader background("background.vs", "background.fs");
...
// grass
    ...
    // ----------------------------------------------------------------------
    ctangulartoCubemap.use();
    ctangulartoCubemap.setInt("equirectangularMap", 0);
    ctangulartoCubemap.setMat4("projection", captureProjection);
    ...
    for (unsigned int i = 0; i < 6; ++i)
    {
        ctangulartoCubemap.setMat4("view", captureViews[i]);
    ...
    // 然后让OpenGL从第一个mip面生成mipmap（对抗可见点伪影）
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /*
        ctangulartoCubemap.use();
        ctangulartoCubemap.setMat4("view", view);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, hdrTexture);
        renderCube();
        */
